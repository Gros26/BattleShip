...
public class GameController {
    ...
    void OnActionNewGame(ActionEvent event) {
        ...
        if (confirmed)
        {
            try {
                ...
                StartView infoView = StartView.getInstance();
                infoView.show();
    ...
    public void handleMousePressed(MouseEvent event) {
        ...
        if (game.getTurn() % 2 == 0)
        {
            ...
            if (machineBoard.getState(x,y) == Board.State.EMPTY) {
                game.setTurn();
                machineBoard.changeState(x, y, Board.State.WATER);
            ...
            if (machineBoard.getState(x,y) == Board.State.OCCUPIED) {
                ...
                for (int i = 0; i < 10; i++) {
                    for (int j = 0; j < 10; j++) {
                        if (machineBoard.getState(i, j) == Board.State.SUNK) {
                        ...
                        } else if (machineBoard.getState(i, j) == Board.State.HIT) {
        ...
        if (game.getTurn() % 2 != 0) {
            ...
            pause.setOnFinished(e -> {
                ...
                do {
                    ...
                    if (playerBoard.getState(w, z) == Board.State.EMPTY) {
                        game.setTurn();
                        playerBoard.changeState(w, z, Board.State.WATER);
                        Path waterHit = drawWaterHit();
                    ...
                    if (playerBoard.getState(w, z) == Board.State.OCCUPIED) {
                        ...
                        for (int i = 0; i < 10; i++) {
                            for (int j = 0; j < 10; j++) {
                                if (playerBoard.getState(i, j) == Board.State.SUNK) {
                                    Path shipSunk = drawShipSunk();
                                    shipSunk.setId("shipSunk");
                                    shipSunk.setLayoutX(i * CELL_SIZE);
                                    shipSunk.setLayoutY(j * CELL_SIZE);
                                    panePosition.getChildren().add(shipSunk);
                                } else if (playerBoard.getState(i, j) == Board.State.HIT) {
                                    Path shipHit = drawShipHit();
                                    shipHit.setId("shipHit");
                                    shipHit.setLayoutX(i * CELL_SIZE);
                                    shipHit.setLayoutY(j * CELL_SIZE);
    ...
    public void drawHitsContinue() {
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                ...
                if (machineBoard.getState(i,j) == Board.State.WATER) {
                    Path waterHit = drawWaterHit();
                ...
                if (playerBoard.getState(i,j) == Board.State.WATER) {
                    Path waterHit = drawWaterHit();
                    waterHit.setLayoutX(i * CELL_SIZE);
                    waterHit.setLayoutY(j * CELL_SIZE);
                    panePosition.getChildren().add(waterHit);
